...
const schema = yup.object({
  ...
  budgetPositionId: yup.string().when('budgetType', {
    is: 'Transfer',
    then: s => s.notRequired().default(''),
    otherwise: s => s.required('Budget position is required'),
  }),
  accountToId: yup.string().when('budgetType', {
    is: 'Transfer',
    then: s => s.required('Account to is required'),
